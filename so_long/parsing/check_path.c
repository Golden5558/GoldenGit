...
/*   Updated: 2025/01/20 23:43:23 by nberthal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**get_map_and_errcheck(char *file, t_point *pos)
{
	...
	if (!map)
		error_exit(-1);
	get_start_and_check_invalid_char(map, pos);
	pos->c_count = 0;
	if (check_map_error(map, line, pos) == -1)
	{
		ft_freeall(map);
		error_exit(3);
	}
	verif_map = copy_map(map, line);
	if (check_path(pos->x, pos->y, verif_map, pos) == 0)
	{
