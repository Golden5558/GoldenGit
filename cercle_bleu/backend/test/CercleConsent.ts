...
describe("MedicalConsentNFT", function () {
  ...
  describe("Consent management", function () {
    ...
    it("Should handle consent granting with comprehensive validation", async function () {
      ...
      const totalConsents = await consentContract.getTotalConsents();
      expect(totalConsents).to.equal(1);

      const totalActiveConsents = await consentContract.getTotalActiveConsents();
      expect(totalActiveConsents).to.equal(1);

      const studyActiveCount = await consentContract.getStudyActiveConsentCount(studyId);
      expect(studyActiveCount).to.equal(1);

      const balance = await consentContract.balanceOf(patient1.address);
      expect(balance).to.equal(1);
    });

    it("Should update getTotalConsents and getStudyActiveConsentCount after revocation", async function () {
      const { consentContract, tokenId, patientId, studyId } = await deployWithOneConsent();

      expect(await consentContract.getTotalConsents()).to.equal(1);
      expect(await consentContract.getStudyActiveConsentCount(studyId)).to.equal(1);

      await consentContract.connect((await ethers.getSigners())[1]).revokeConsent(tokenId, patientId);

      // Total consents count does not decrease (historical tracking)
      expect(await consentContract.getTotalConsents()).to.equal(1);
      // Active count decreases
      expect(await consentContract.getTotalActiveConsents()).to.equal(0);
      expect(await consentContract.getStudyActiveConsentCount(studyId)).to.equal(0);
    });
    ...
    it("Should reject selfGrantConsent from an unregistered patient", async function () {
      const { consentContract, patient2, studyId, datasetHash } = await deployConsentFixture();
      // patient2 is not registered

      await expect(consentContract.connect(patient2).selfGrantConsent(datasetHash, studyId, validityDuration))
        .to.be.revertedWithCustomError(consentContract, "PatientNotRegistered");
    });
