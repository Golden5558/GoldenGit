...
async function main(): Promise<void> {
  ...
  const CercleConsent = await ethers.deployContract("CercleConsent");
 ...
 // const hasEtherscanKey = vars.get("ETHERSCAN_API_KEY");
  ...
  if (!isLocalhost) {
    ...
    await CercleConsent.deploymentTransaction()?.wait(5);
  }
  ...
  if (!isLocalhost) {
    ...
    await verify(CercleConsent.target.toString());
  }
}
