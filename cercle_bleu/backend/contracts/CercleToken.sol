...
contract CercleToken is ERC20 {
    ...
    /// @notice Tracks whether a patient has already been rewarded for a given dataset hash
    mapping(address => mapping(bytes32 => bool)) public rewardedDatasets;
    ...
    /// @notice Error thrown when an unauthorized user attempts an action
    ...
    /// @notice Error thrown when a patient tries to claim a reward for an already-rewarded dataset
    error AlreadyRewarded(address patient, bytes32 datasetHash);
     ...
     * @dev Mints 50 CERCLE tokens to the patient for each download 
    ...
    function rewardForDataDownload(address patient, bytes32 datasetHash)
        ...
        onlyAuthorizedUsers()
    {
        if (rewardedDatasets[patient][datasetHash]) revert AlreadyRewarded(patient, datasetHash);
        rewardedDatasets[patient][datasetHash] = true;
