...
import { OAuthProviderID, WalletChainType, SmartWalletType, CustomOAuthProviderID, User as User$1, AppResponse, PasskeyAuthenticateRequestBody } from '@privy-io/api-types';
...
/**
 * Plugin that keeps user state in sync across browser tabs using BroadcastChannel.
 *
 * @experimental This plugin is experimental and may change or be removed in future versions.
 */
type CrossTabUserSyncPlugin = {
    readonly id: typeof CROSS_TAB_USER_SYNC_PLUGIN_ID;
};
/**
 * Creates a cross-tab user sync plugin.
 *
 * @experimental This plugin is experimental and may change or be removed in future versions.
 *
 * @param options.channelId - A unique channel ID for your app.
 *
 * @example
 * ```tsx
 * createCrossTabUserSyncPlugin({
 *   channelId: process.env.NEXT_PUBLIC_PRIVY_SYNC_CHANNEL_ID
 * })
 * ```
 */
declare function createCrossTabUserSyncPlugin(options: {
    channelId: string;
}): CrossTabUserSyncPlugin;

/**
 * Plugin that adds data suffix support to the SDK.
 */
type DataSuffixPlugin = {
    readonly id: typeof DATA_SUFFIX_PLUGIN_ID;
    /**
     * Appends a data suffix to transaction data.
     *
     * @param opts.data - The current transaction data (if any)
     * @returns The transaction data with the suffix appended
     */
    appendDataSuffix: (opts: {
        data: Hex;
    }) => Hex;
};
/**
 * Creates a data suffix plugin instance with the configured suffix.
 *
 * @param suffix - The hex suffix to append to transaction data (with or without '0x' prefix).
 *                 Must be valid hex data.
 * @returns A plugin instance that transforms transaction data.
 * @throws {Error} If the suffix is not valid hex.
 */
declare function createDataSuffixPlugin(suffix: string): DataSuffixPlugin;

/**
 * Plugin that overrides automatic embedded wallet creation (createOnLogin config).
 *
 * When present, the plugin's `shouldCreateWallet` predicate fully overrides the app's
 * createOnLogin setting: `true` = create wallet(s), `false` = do not create.
 * When the plugin is not registered, the default createOnLogin logic is used.
 *
 * @experimental This plugin is experimental and may change or be removed in future versions.
 */
type WalletCreationFilterPlugin = {
    readonly id: typeof WALLET_CREATION_ON_LOGIN_PLUGIN_ID;
    /**
     * Override for whether to create embedded wallet(s) for this user.
     * When the plugin is active, this replaces the createOnLogin config.
     */
    shouldCreateWallet: ({ user }: {
        user: User;
    }) => boolean;
};
/**
 * Creates a plugin that overrides automatic embedded wallet creation based on a
 * user-defined predicate.
 *
 * When the plugin is registered, it overrides the app's createOnLogin config:
 * - `shouldCreateWallet({ user }) === true` → create embedded wallet(s) for this user
 * - `shouldCreateWallet({ user }) === false` → do not create
 *
 * @experimental This plugin is experimental and may change or be removed in future versions.
 *
 * @param options.shouldCreateWallet Predicate receiving `{ user }`; true = create, false = do not create (overrides createOnLogin).
 *
 * @example
 * ```tsx
 ...
 * ```
 */
declare function createWalletCreationOnLoginPlugin(options: {
    shouldCreateWallet: ({ user }: {
        user: User;
    }) => boolean;
}): WalletCreationFilterPlugin;
...
/** @deprecated rabby_wallet is no longer supported */
 | 'rabby_wallet' | 'bybit_wallet' | 'ronin_wallet' | 'haha_wallet' | 'safe' | 'solflare' | 'backpack' | 'jupiter' | 'binance' | 'binanceus' | 'bitget_wallet' | 'kraken_wallet';
...
interface PrivyClientConfig {
    ...
    /**
     * Plugins to register with the SDK. Plugins extend SDK functionality
     * and are automatically registered when the PrivyProvider mounts.
     *
     * @example
     * ```tsx
     ...
     *   appId="..."
     *   config={{
     *     plugins: [dataSuffix('0x1234abcd')]
     *   }}
     * >
     * ```
     */
    plugins?: Array<DataSuffixPlugin | CrossTabUserSyncPlugin | WalletCreationFilterPlugin>;
    /**
...
type AppConfig = {
    ...
    globalDisablePasskeys: NonNullable<PrivyClientConfig['globalDisablePasskeys']>;
    ...
    /**
     * Cross-app providers specified in the loginMethods array (format: privy:${appId})
     */
    crossAppProviders: string[];
    /**
    ...
    captcha: {
        /**
         * Null if captcha is disabled, otherwise the enabled captcha provider.
         */
        enabledProvider: AppResponse['enabled_captcha_provider'];
        siteKey: NonNullable<AppResponse['captcha_site_key']>;
    };
    externalWallets: {
        coinbaseWallet: {
    ...
    /** Enable debug logging for wallet connectors */
...
type SignerInput = SessionSignerInput;
