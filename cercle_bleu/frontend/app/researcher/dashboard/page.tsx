...
export default function ResearcherDashboard() {
  ...
  const { userData, isLoadingUser, userType, refreshUserData, isAuthenticated, isAuthenticating } = useAuthContext();
  ...
  // Cast userData to ResearcherInfo type
  const researcherInfo = userType === 'researcher' ? userData as ResearcherInfo | null : null;
  const isCheckingDatabase = isLoadingUser;
  const needsRegistration = userType === 'unknown';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {researcherInfo && (
          <FirstRunChecklist
            storageKey={`researcher_${researcherInfo.id}`}
            title="Vos premières étapes"
            tasks={[
              {
                id: 'study',
                label: 'Créer votre première étude',
                href: '/researcher/studies',
                autoCompleted: createdStudiesCount > 0,
              },
              {
                id: 'data',
                label: 'Explorer les datasets disponibles',
