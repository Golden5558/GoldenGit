...
export default function MeasurementUpload() {
  ...
  const { userData, isLoadingUser } = useAuthContext();
  ...
  const patientInfo: PatientInfo | null = userData
    ? { id: userData.id as number, firstName: userData.firstName, lastName: userData.lastName }
    : null;

  // Charger les études depuis l'API publique
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validMeasurements = measurements.filter(
      (m) => m.measurementType && m.value > 0 && m.timestamp
    );
    ...
    try {
      ...
      const normalizedMeasurements = validMeasurements.map((m) => ({
        ...m,
      ...
      const payload = {
        ...
        datasetHash,
        measurements: normalizedMeasurements.map((m) => ({
      ...
      const response = await authenticatedFetch('/api/patient/upload', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        toast.error(data.error || 'Erreur lors de la sauvegarde des données');
      }
    } catch (error) {
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <Button type="submit" disabled={isLoading || isAuthenticating} className="flex-1">
