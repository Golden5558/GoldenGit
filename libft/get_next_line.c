...
/*   Updated: 2025/01/30 10:41:24 by nberthal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	read_line(int fd, t_list *list)
{
	...
	while (!check_stock(list) && list->read_size != 0)
	{
		...
		list->stock = ft_strjoin_and_free(list->stock, list->buffer);
...
char	*get_next_line(int fd)
{
	...
	if (*remnant)
	{
		list.stock = ft_substr_free(remnant, 0, ft_strlen(remnant), list.stock);
		list.len_stock = ft_strlen(list.stock);
